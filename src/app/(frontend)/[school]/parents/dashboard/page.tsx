...
export default async function ParentsDashboardPage({
  ...
  // Fetch aggiornamenti per il genitore
  ...
  // Fetch appuntamenti per il genitore
              ...
              {updates.docs.length === 0 ? (
                <div className="bg-muted/30 p-12 rounded-2xl text-center border-2 border-dashed">
                  ...
                  <p className="text-sm text-muted-foreground mt-2">
                    Torna a controllare presto per nuove attività!
                  </p>
                </div>
              ) : (
                <div className="space-y-8">
                  {updates.docs.map((update) => (
                    <SpotlightCard
                      key={update.id}
                      className="bg-card/50 backdrop-blur-sm border-muted/50"
                    >
                      <div className="flex items-start justify-between mb-6">
                        ...
                        <span
                          className={`inline-flex items-center rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wider shadow-sm
                            ${
                            ...
                            {update.photos.map((photo, index) => {
                              if (typeof photo === 'object' && photo.url) {
                                return (
                                  <div
                                    key={index}
                                    className="aspect-square rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 group"
                                  >
                                    <Image
                                      src={photo.url}
                                      alt={`Foto ${index + 1}`}
