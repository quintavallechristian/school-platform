...
export const SendCommunicationField: React.FC = () => {
  const schoolField = useFormFields(([fields]) => fields.school)
  const { user } = useAuth()

  // Usa useField per avere accesso a setValue
  const { value, setValue } = useField<boolean>({ path: 'sendCommunication' })
  ...
  useEffect(() => {
    const checkFeature = async () => {
      ...
      try {
        ...
        setIsFeatureEnabled(communicationsEnabled)

        if (!communicationsEnabled) {
          setErrorMessage('Attiva le comunicazioni nelle impostazioni della scuola.')
  ...
  return (
    <div className="field-type checkbox">
      <div style={{ marginTop: '8px' }}>
        ...
        >
          {isLoading ? 'Verifica in corso...' : 'Invia comunicazione?'}
        </span>
      </div>
...
export default SendCommunicationField
